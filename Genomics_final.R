...
## Q2 Association entre variables cliniques et données morphologiques
clinical_data <- read.delim("clinical_data.tsv")
...
# Analyse des corrélation entre les variables cliniques et les différents clusters
clinical_corr <- subset(mat_cor, select = -c(SEX, AGE, HGHT, WGHT, BMI, TRISCHD, DTHVNT, DTHHRDY))
clinical_corr <- clinical_corr[1:8,]
clinical_corr <- t(clinical_corr)

# Création d'un tableau avec les corrélation avec les clusters > 0.1 en valeur abs. pour toutes les variables
variables <- c("SEX", "AGE", "HGHT", "WGHT", "BMI")

for (var in variables) {
  vec <- sort(clinical_corr[, var], decreasing = TRUE)
  vec <- vec[abs(vec) >= 0.1]
  df <- data.frame(cluster = names(vec), correlation = vec)
  assign(paste0("corr_", var), df, envir = .GlobalEnv)
}

xtable_df <- xtable(corr_BMI)
print(xtable_df, include.rownames=FALSE)


# Exemple d'analyse différentielle : ~ SEX + DTHHRDY
dds <- DESeqDataSetFromMatrix(countData = t(morph_counts),
                              colData = clinical_data,
                              design = ~ SEX + DTHHRDY)
dds <- DESeq(dds)
results <- results(dds)
plotMA(results, main="MA-Plot : SEX + DTHHRDY", ylim=c(-2, 2), cex=0.8)

top_SEX_DTHHRDY <- head(subset(results, padj < 0.05), 10)
cluster_names <- rownames(top_SEX_DTHHRDY)

data_SEX_DTHHRDY <- data.frame(
  Variable = "SEX + DTHHRDY",
  Number_of_Regulated_Clusters = nrow(top_SEX_DTHHRDY),
  Top_10_Regulated_Clusters = paste(cluster_names, collapse="\n"),
  p_value = paste(top_SEX_DTHHRDY$pvalue, collapse="\n")
)
data_SEX_DTHHRDY


# Construction de la fonction d'analyse différentielle
diff_analysis <- function(countData, colData, disign){
  ...
  plotMA(results, main=paste0("MA-Plot - ", disign), ylim=c(-2, 2), cex=0.8) 
...
variables <- c("TRISCHD", "DTHVNT", "DTHHRDY")
...
final_table_1 <- as.data.frame(final_table)

xtable_df <- xtable(final_table_1)
print(xtable_df)

kable(final_table_1)
write.csv(final_table_1, "DE_Analysis_Summary_technicals.csv")


# Ajustement des variables démographiques/santé avec les variables confondantes
# Avec TRISCHD
final_table <- data.frame(
  Variable = character(),
  Number_of_Regulated_Clusters = integer(),
...
variables <- c('AGE + TRISCHD + DTHHRDY' ,'SEX + DTHHRDY','WGHT + TRISCHD + DTHHRDY','HGHT + TRISCHD + DTHHRDY', 'BMI + TRISCHD + DTHHRDY')
for (i in 1:length(variables)){
  print(variables[i])
  data <- diff_analysis(morph_counts,clinical_data,variables[i])
  final_table <- rbind(final_table, data)
...
final_table_2 <- as.data.frame(final_table)

xtable_df <- xtable(final_table_2)
print(xtable_df)

kable(final_table_2)
write.csv(final_table_2, "DE_Analysis_Summary_confoundants_with_trischd.csv")
...
variables <- c('AGE + DTHHRDY','SEX + DTHHRDY','WGHT + DTHHRDY','HGHT + DTHHRDY', 'BMI + DTHHRDY')
for (i in 1:length(variables)){
  print(variables[i])
  data <- diff_analysis(morph_counts,clinical_data,variables[i])
  final_table <- rbind(final_table, data)
...
final_table_3 <- as.data.frame(final_table)

xtable_df <- xtable(final_table_3)
print(xtable_df)

kable(final_table_3)
write.csv(final_table_3, "DE_Analysis_Summary_confoundants.csv")
